...
public class EpisodeDetailFragment extends Fragment
    ...
    private static final String[] EPISODE_COLUMNS = {
            ...
            WtaContract.EpisodeEntry.COLUMN_SEASON_NUMBER,
            WtaContract.EpisodeEntry.COLUMN_TITLE,
            WtaContract.EpisodeEntry.COLUMN_SCREENSHOT_PATH,
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        mFabWatched
                .setOnClickListener(new WtaEpisodeFabOnClickListener(WtaContract.EpisodeEntry.COLUMN_WATCHED));
        mFabWatchlist
    ...
    private void onDetailEpisodeLoadFinished(Cursor data) {
        ...
        if (screenshotPath!=null) {
            Glide.with(getActivity()).load(screenshotPath)
                    .transition(withCrossFade()).into(mEpisodeScreenshot);
        } else {
    ...
    private class WtaEpisodeFabOnClickListener implements View.OnClickListener {
        ...
        public void onClick(View view) {
            int value = 0;
            if (mColumnName.equals(WtaContract.EpisodeEntry.COLUMN_WATCHED)) {
                mIsWatched = !mIsWatched;
                value = mIsWatched ? 1 : 0;
            } else if (mColumnName.equals(WtaContract.EpisodeEntry.COLUMN_WATCHLIST)) {
                mIsWatchlist = !mIsWatchlist;
                value = mIsWatchlist ? 1 : 0;
            }

            ContentValues episode = new ContentValues();
            episode.put(WtaContract.EpisodeEntry._ID, mEpisodeId);
            episode.put(mColumnName, value);
            getActivity().getContentResolver()
                    .update(WtaContract.EpisodeEntry.CONTENT_URI,
                            episode,
                            WtaProvider.sEpisodeSelection,
